<template>
  <aside class="sidebar">
    ...
    <nav :aria-label="tr('后台导航','Admin navigation')">
      <a v-for="item in items" :key="item.path" :href="'#' + item.path" :class="{ active: isActive(item.path) }" @click.prevent="$emit('navigate', item.path)">
    ...
    <div class="sidebar-footer">
      ...
      <a class="site-link" :href="assetUrl('')" target="_blank" rel="noopener noreferrer"><AppIcon name="external"/><span>{{ tr('查看站点','View site') }}</span></a>
...
const items = computed(() => [
  ...
  { path: '/publishing', label: tr('发布中心','Publishing'), icon: 'publishing' }, { path: '/essays', label: tr('随笔管理','Essays'), icon: 'essays' }, { path: '/media', label: tr('媒体资源','Media'), icon: 'media' }, { path: '/recovery', label: tr('恢复中心','Recovery'), icon: 'trash' }, { path: '/trash', label: tr('回收站','Trash'), icon: 'trash' }, { path: '/config', label: tr('站点配置','Settings'), icon: 'config' }, { path: '/themes', label: tr('主题外观','Themes'), icon: 'themes' }, { path: '/plugin', label: tr('关于插件','About plugin'), icon: 'plugin' }
]);
