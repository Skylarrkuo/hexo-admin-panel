<template>
  <div class="config-atlas">
    ...
    <div class="config-atlas-main">
      ...
      <div class="config-note">{{ tr('完整表单会保留值的布尔、数字、字符串、空值及数组类型。表单保存会重新排版 YAML；如需保留原注释，请使用“YAML 编辑”。','The form preserves boolean, number, string, null, and array values. Saving reformats YAML; use “YAML editor” to preserve comments.') }}</div>
...
function updateNumber(field,value){if(value===''){errors[field.key]=tr('数字不能为空','A number is required');return;}const parsed=Number(value);if(Number.isFinite(parsed))setValue(field,parsed);}
