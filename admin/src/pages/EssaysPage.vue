<template>
  <section>
    ...
    <div v-if="editing&&!batchOpen" class="card essay-editor">
      ...
      <div class="form-group"><label>{{ tr('发布时间','Publish date') }} · {{siteZone}}</label><input v-model="form.date" type="datetime-local" step="1"></div>
    ...
    <div v-if="items.length" class="toolbar"><label><input type="checkbox" :checked="items.length>0&&selected.length===items.length" :disabled="batchOpen||items.length>100" @change="selected=$event.target.checked?items.map(item=>item.id):[]"> {{tr('全选','Select all')}}</label><span class="text-sm text-muted">{{tr('已选 {count} 条 · 每批最多 100 条','{count} selected · up to 100 per batch',{count:selected.length})}}</span><button v-if="selected.length" class="btn btn-outline btn-sm" :disabled="batchOpen" @click="selected=[]">{{tr('清空选择','Clear selection')}}</button></div>
    ...
    <div v-else class="essay-list">
      <article v-for="item in items" :key="item.id" class="card essay-item">
        <div class="essay-meta"><label><input v-model="selected" type="checkbox" :value="item.id" :disabled="batchOpen||(!selected.includes(item.id)&&selected.length>=100)" :aria-label="tr('选择随笔 {date}','Select essay {date}',{date:item.date})"> <time>{{ item.date }}</time></label><div class="btn-group"><button class="btn btn-outline btn-sm" :disabled="batchOpen||saving" @click="startEdit(item)">{{ tr('编辑','Edit') }}</button><button class="btn btn-danger btn-sm" :disabled="batchOpen||saving||editing" @click="remove(item)">{{ tr('删除','Delete') }}</button></div></div>
...
function startEdit(item){if(!draft.leave())return;draft.pause();Object.assign(form,{id:item.id,content:item.content,date:inputDate(item.date)});editing.value=true;draft.loaded();window.scrollTo({top:0,behavior:'smooth'});}
function cancel(){if(!draft.leave())return;draft.pause();editing.value=false;Object.assign(form,{id:'',content:'',date:''});draft.loaded();}
async function save(){if(!form.content.trim()){notify(tr('随笔内容不能为空','Essay content is required'),'error');return;}if(!form.date){notify(tr('请选择发布时间','Choose a publish date'),'error');return;}saving.value=true;try{const body={content:form.content,date:apiDate(form.date),revision:revision.value};const submitted=draft.snapshot();const data=form.id?await api.put('/essays/'+encodeURIComponent(form.id),body):await api.post('/essays',body);revision.value=data.revision;submitted.revision=data.revision;form.id=data.item.id;submitted.form.id=data.item.id;draft.saved(submitted);items.value=[...items.value.filter(item=>item.id!==data.item.id),data.item].sort((a,b)=>b.date.localeCompare(a.date));if(draft.dirty.value){notify(tr('内容已保存，后续修改仍保留','Content saved; newer edits remain'),'info');return;}notify(form.id?tr('随笔已更新','Essay updated'):tr('随笔已保存','Essay saved'),'success');cancel();await load();}catch(error){notify(errorMessage(error),'error');}finally{saving.value=false;}}
