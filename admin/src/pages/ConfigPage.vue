...
async function save(){if(type.value==='theme'&&view.value==='form'&&!valid.value){notify(tr('请先修正 JSON 格式错误','Fix the JSON format errors first'),'error');return;}saving.value=true;try{const body=view.value==='raw'?{raw:raw.value,type:type.value}:{data:current(),type:type.value};await api.put('/config',body);if(showBackups.value)loadBackups();emit('saved-restart');}catch(error){notify(errorMessage(error),'error');}finally{saving.value=false;}}
