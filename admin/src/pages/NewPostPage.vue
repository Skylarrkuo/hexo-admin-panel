...
const {tr,errorMessage}=useI18n();const emit=defineEmits(['create','cancel','notify','dirty-change']);
const siteZone=ref('UTC');const scaffolds=ref([]);const form=reactive({title:'',scaffold:'post',workflowStatus:'draft',publishAt:'',maxAttempts:3,retryDelayMinutes:5});
const draft=useLocalDraft({key:()=> 'new_post',state:()=>({...form}),apply:value=>Object.assign(form,value),emit});defineExpose({saved:draft.saved});
...
function submit(){try{if(canSubmit.value)emit('create',{...form,title:form.title.trim(),publishAt:form.publishAt?siteInputToIso(form.publishAt,siteZone.value):''});}catch(error){emit('notify',errorMessage(error),'error');}}
onMounted(async()=>{try{siteZone.value=(await api.get('/native')).timeZone||'UTC';}catch(_){}form.publishAt=localDate(new Date(Date.now()+3600000));try{scaffolds.value=(await api.get('/scaffolds')).items||[];if(!scaffolds.value.some(item=>item.name==='post'))form.scaffold=scaffolds.value[0]?.name||'';}catch(error){emit('notify',errorMessage(error),'error');}finally{draft.loaded();}});
</script>
