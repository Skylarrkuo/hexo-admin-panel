  ...
  <section>
    ...
    <div v-else>
      ...
      <div v-else class="media-grid"><div v-for="file in files" :key="file.name" class="media-item">
        ...
        <template v-else><div class="info" :title="file.name">{{ file.name }}</div><div class="info text-muted">{{ formatSize(file.size) }} · <span :title="referenceTitle(file)">{{ file.used?tr('{count} 处引用','{count} references',{count:file.referenceCount}):tr('未使用','Unused') }}</span></div>
...
const usageOptions=computed(()=>[{value:'all',label:tr('全部','All')},{value:'used',label:tr('已使用','Used')},{value:'unused',label:tr('未使用','Unused')}]);
