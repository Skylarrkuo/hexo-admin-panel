<template>
  <section class="post-editor-page">
    <ContentHistory ref="history" :source="historySource" :endpoint="'/posts/'+postId" @notify="notify" @reload="load" @merge="mergeContent"/><PostResources :post-id="postId" @notify="notify" @insert="insertAtCursor"/>
    <div class="toolbar editor-commandbar">
      ...
      <div class="btn-group"><span v-if="autosaveStatus" class="text-sm text-muted">{{ autosaveStatus }}</span><button class="btn btn-outline" @click="realPreview" :disabled="saving||previewing">{{ previewing?tr('构建预览中...','Building preview...'):tr('真实主题预览','Real theme preview') }}</button><button class="btn btn-outline" :disabled="saving" @click="saveAndPublish">{{tr('保存并进入发布流程','Save and open Publishing')}}</button><button class="btn btn-primary" @click="save()" :disabled="saving">{{ saving?tr('保存中...','Saving...'):tr('保存文章','Save post') }}</button></div>
    </div>
      ...
      <template v-if="mode==='visual'">
        <div class="card post-meta-card">
          ...
          <div class="form-row">
            ...
            <div class="form-group"><label>{{ tr('发布日期','Publish date') }} · {{siteZone}}</label><input v-model="editor.date" type="datetime-local" @input="markVisualDirty"></div>
          </div>
...
import PostResources from '../components/PostResources.vue';
...
const props=defineProps({postId:{type:String,required:true}});const emit=defineEmits(['cancel','saved','notify','dirty-change','publish']);
...
function mergeContent(data){editor.sourceContent=data.raw;editor.revision=data.revision;baseRaw.value=data.base;editor.sourceDirty=true;mode.value='source';parseSource();scheduleAutosave();}
function notify(message,type='info'){emit('notify',message,type);}function toLocalDate(value){try{return siteInput(value,siteZone.value);}catch(_){return String(value||'').replace(' ','T').slice(0,16);}}
...
function persistLocalDraft(){if(!setDirtyState()){clearLocalDraft();return;}try{localStorage.setItem(autosaveKey.value,JSON.stringify({version:1,savedAt:new Date().toISOString(),baseRevision:editor.revision,baseRaw:baseRaw.value,mode:mode.value,viewMode:viewMode.value,state:contentState()}));autosaveStatus.value=tr('已自动保存在本机','Autosaved locally');}catch(_){autosaveStatus.value=tr('本地自动保存失败','Local autosave failed');}}
...
async function load(){try{siteZone.value=(await api.get('/native')).timeZone||'UTC';}catch(_){}loading.value=true;applying=true;try{const [data,taxonomies]=await Promise.all([api.get('/posts/'+props.postId),api.get('/taxonomies')]);editor.id=data._id;baseRaw.value=data.raw;historySource.value=data.historySource||'source/'+data.source;applyData(data);editor.sourceContent=data.raw||'';editor.sourceDirty=false;taxonomyOptions.categories=taxonomies.categories||[];taxonomyOptions.tags=taxonomies.tags||[];baseline=contentSignature();}catch(error){notify(errorMessage(error),'error');emit('cancel');}finally{applying=false;loading.value=false;}restoreLocalDraft();}
...
async function save(stay=false){if(mode.value==='visual'&&!editor.title.trim()){notify(tr('标题不能为空','Title is required'),'error');return false;}saving.value=true;try{const payload=mode.value==='source'?{raw:editor.sourceContent,revision:editor.revision}:{...body(),revision:editor.revision};const savedSignature=contentSignature();const data=await api.put('/posts/'+editor.id,payload);editor.revision=data.revision;baseRaw.value=data.raw||payload.raw||baseRaw.value;refreshFailed.value=data.refreshed===false;baseline=savedSignature;const hasNewEdits=setDirtyState();if(hasNewEdits)persistLocalDraft();else clearLocalDraft();if(refreshFailed.value){notify(tr('内容已保存到文件，但 Hexo 刷新失败。请检查服务日志，修复后重建站点；无需重复提交内容。','Content was saved to disk, but Hexo refresh failed. Check the server logs and rebuild after fixing the error; the content does not need to be resubmitted.'),'warning');return false;}if(hasNewEdits){notify(tr('提交时的内容已保存，后续修改仍保留在编辑器','Submitted content saved; newer edits remain in the editor'),'info');return false;}if(!stay){notify(tr('文章已更新','Post updated'),'success');emit('saved');}return true;}catch(error){persistLocalDraft();if(error.status===409)await history.value.capture(baseRaw.value,mode.value==='source'?editor.sourceContent:(await api.post('/posts/source/build',body())).raw);else notify(errorMessage(error),'error');return false;}finally{saving.value=false;}}
...
function beforeUnload(event){persistLocalDraft();if(!setDirtyState())return;event.preventDefault();event.returnValue='';}
watch(()=>props.postId,load,{immediate:true});
watch(()=>contentState(),scheduleAutosave,{deep:true});
onMounted(()=>window.addEventListener('beforeunload',beforeUnload));
onBeforeUnmount(()=>{persistLocalDraft();window.removeEventListener('beforeunload',beforeUnload);clearTimeout(previewTimer);clearTimeout(autosaveTimer);});
</script>
