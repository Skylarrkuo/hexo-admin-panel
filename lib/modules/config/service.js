...
function createConfigService(context) {
  ...
  return {
    ...
    save(type, body) {
      ...
      requireRevision(body.revision, currentRaw, {
      ...
      if (body.raw !== undefined) {
        ...
        nextRaw = dumpYaml(deepMerge(existing, body.data));
      } else throw badRequest('No config content provided');
      files.writeText(target.writePath, nextRaw);
      return { backupId, revision: contentRevision(nextRaw) };
    },
    listBackups(type) {
