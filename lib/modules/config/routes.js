...
function configRoutes(context) {
  ...
  return [
    ...
    { method: 'PUT', path: '/config', async handler({ req, res, query }) { const body=validate(await readJson(req),{type:{type:'string',enum:['site','theme']},raw:{type:'string'},data:{type:'object'}});success(res,config.save(validateConfigType(body.type||query.type),body)); } },
    ...
    { method: 'POST', path: '/config/backups/:id/restore', handler({ res, params }) { success(res,config.restoreBackup(validateId(params.id))); } }
  ];
}
