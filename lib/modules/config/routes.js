...
function configRoutes(context) {
  ...
  return [
    ...
    {method:'POST',path:'/config/preview',async handler({req,res}){const body=validate(await readJson(req),{type:{type:'string',enum:['site','theme']},raw:{type:'string'},data:{type:'object'},replace:{type:'boolean'},unset:{type:'array',maxItems:500,items:{type:'string'}},revision:{type:'string',required:true,pattern:/^[a-f0-9]{64}$/}});success(res,config.preview(validateConfigType(body.type),body));}},
    { method: 'PUT', path: '/config', async handler({ req, res, query }) { const body=validate(await readJson(req),{type:{type:'string',enum:['site','theme']},raw:{type:'string'},data:{type:'object'},replace:{type:'boolean'},unset:{type:'array',maxItems:500,items:{type:'string'}},revision:{type:'string',required:true,pattern:/^[a-f0-9]{64}$/}});success(res,config.save(validateConfigType(body.type||query.type),body)); } },
