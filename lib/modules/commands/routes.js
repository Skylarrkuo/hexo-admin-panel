...
function commandRoutes(context) {
  ...
  routes.push({
    ...
    async handler({ req, res }) {
      ...
      const currentServer = req.socket && req.socket.server;
      const result = await restart(context, port, context.runtime, currentServer);
      jobs.note(job.id, '新的 Hexo 服务进程已安排以前台监护模式启动');
