...
function parsePageSource(raw) {
  ...
  return {
    title: parsed.title || '', date: parsed.date || '', layout: parsed.layout || 'page',
...
function createPageService(context) {
  ...
  return {
    ...
    get(id) {
      ...
      return { id, path: found.relative, ...parsePageSource(raw), raw, revision: contentRevision(raw), permalink: record(found.filePath).permalink };
    },
    ...
    async create(data) {
      ...
      const values = { title: data.title || path.basename(relative, '.md'), date: data.date || new Date().toISOString().replace('T', ' ').slice(0, 19), slug: relative.split('/')[0] };
      ...
      await refresh();
      return { id: pageId(relative), path: relative, revision: contentRevision(raw) };
    },
    async update(id, data) {
      ...
      await refresh();
      return { id, path: found.relative, revision: contentRevision(raw) };
    },
