...
function previewRoutes(context) {
  ...
  return [
    ...
    { method: 'GET', path: '/previews/:token/*file', public: true, handler({ res,params }) {
      ...
      const headers={...SECURITY_HEADERS,'X-Frame-Options':'SAMEORIGIN','Content-Security-Policy':"default-src 'self' data: blob: https: http:; script-src 'self' 'unsafe-inline' 'unsafe-eval' https: http:; style-src 'self' 'unsafe-inline' https: http:; img-src 'self' data: blob: https: http:; font-src 'self' data: https: http:; frame-ancestors 'self'; connect-src 'self' https: http: ws: wss:",'Content-Type':result.contentType,'Cache-Control':'no-store'};
