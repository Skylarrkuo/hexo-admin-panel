...
const HISTORY_STATES = new Set(['completed', 'cancelled', 'failed']);
...
function createScheduleService(context) {
  ...
  function persist() {
    ...
    const retainedHistory = new Set(items.filter(item => HISTORY_STATES.has(item.status))
      .sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt))).slice(0, 200));
    const retained = items.filter(item => !HISTORY_STATES.has(item.status) || retainedHistory.has(item));
    files.writeText(statePath, JSON.stringify({ version: 2, items: retained }, null, 2));
    items = retained;
  }
  ...
  const service = {
    list(query) {
      ...
      let timeZone;
      try { timeZone = new Intl.DateTimeFormat('en', { timeZone: context.hexo.config && context.hexo.config.timezone || undefined }).resolvedOptions().timeZone; }
      catch (_) { timeZone = 'UTC'; }
      return { items: result, total: result.length, timeZone };
    },
    forPost(postId) {
