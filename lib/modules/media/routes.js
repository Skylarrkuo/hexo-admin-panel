...
function mediaRoutes(context) {
  ...
  return [
    ...
    { method:'GET',path:'/media/:filename/delete-preview',handler({res,params}){success(res,media.previewDelete(validateId(params.filename,'filename')));} },
    ...
    { method: 'POST', path: '/media/:filename/rename-preview', async handler({ req, res, params }) { const body=validate(await readJson(req),{name:{type:'string',required:true,minLength:1,maxLength:255}});success(res,media.previewRename(validateId(params.filename,'filename'),body.name)); } },
    { method: 'PUT', path: '/media/:filename/rename', async handler({ req, res, params }) { const body=validate(await readJson(req),{name:{type:'string',required:true,minLength:1,maxLength:255},revision:{type:'string',pattern:/^[a-f0-9]{64}$/}});success(res,await media.rename(validateId(params.filename,'filename'),body.name,body.revision)); } },
    ...
    { method: 'DELETE', path: '/media/:filename', async handler({ req, res, params }) { success(res,await media.remove(validateId(params.filename,'filename'),req.headers['if-match'])); } }
  ];
}
