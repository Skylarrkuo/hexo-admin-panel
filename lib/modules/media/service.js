...
const {sitePath}=require('../../shared/hexo-native');
  ...
  return {
    list(query) {
      ...
      const mediaFiles = allMedia().map(entry => {
        ...
        return { name: entry.relative, path: sitePath(context.hexo.config,'images/'+entry.relative), size: stat.size, modified: stat.mtime.toISOString(), type: mimeFor(entry.relative), ...refs };
      }).filter(file => (!search || file.name.toLowerCase().includes(search)) && (usage === 'all' || (usage === 'used') === file.used) && (!query.source || file.references.some(item => item.source === query.source))).sort((a, b) => new Date(b.modified) - new Date(a.modified));
    ...
    async upload(uploads) {
      ...
      try {
        for (const file of uploads) {
          ...
          result.push({ name: filename, path: sitePath(context.hexo.config,'images/'+filename), size: file.buffer.length });
        }
        await refresh();
      } catch (error) {
    ...
    previewDelete(filename) {
      const plan=renamePlan(filename,filename);
      const references=analysis().items.find(item=>item.name===plan.filename)?.references||[];
      return {name:plan.filename,revision:plan.revision,references,scanScope:SCAN_SCOPE};
    },
    async remove(filename, revision) {
      const plan=this.previewDelete(filename);
      if(plan.references.length && !revision)throw preconditionRequired('请先查看删除影响');
      if(revision && revision!==plan.revision)throw conflict('资源或引用已变化，请重新查看删除影响','MEDIA_DELETE_CONFLICT');
      filename = normalizeMediaPath(filename);
      const filePath = absoluteMediaPath(filename);
      if (!files.exists(filePath)) throw notFound('File not found');
      return context.services.trash.move('media', filePath, { path: sitePath(context.hexo.config,'images/'+filename) });
    },
    ...
    async rename(filename, requestedName, revision) {
      return context.operations.run(async () => {
        ...
        if (plan.nextName === plan.filename) return { name: plan.filename, path: sitePath(context.hexo.config,'images/'+plan.filename) };
        ...
        return { ...presentPlan(plan), path: sitePath(context.hexo.config,'images/'+plan.nextName), backupId };
      });
    },
