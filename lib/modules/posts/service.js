...
const { newPostName, sourceDate } = require('../../shared/hexo-native');
...
function createPostService(context) {
  ...
  function detail(post) {
    ...
    return {
      _id: post._id, title: editable.title, slug: post.slug, date: sourceDate(raw,editable.date),
      ...
      frontMatter: editable.frontMatter, source: post.source, historySource: path.relative(hexo.base_dir, post.full_source).replace(/\\/g, '/'), raw, revision: contentRevision(raw)
    };
  }
  ...
  async function publishRecord(post, raw, published, options) {
    ...
    const assetSource=post.full_source.slice(0,-path.extname(post.full_source).length);
    const assetDestination=destination.slice(0,-path.extname(destination).length);
    const movingAssets=moving && hexo.config.post_asset_folder && files.exists(assetSource);
    if(movingAssets && files.exists(assetDestination))throw conflict('目标文章资源目录已存在','POST_ASSET_CONFLICT');
    ...
    try {
      ...
      if (moving) {
        ...
        moved = true;
        if(movingAssets){files.move(assetSource,assetDestination);assetsMoved=true;}
      }
      if (!(options && options.skipRefresh)) await refresh();
    } catch (error) {
      try {
        if(assetsMoved)files.move(assetDestination,assetSource);
  ...
  return {
    ...
    assetInfo(id) { const post=find(id);return { filePath:post.full_source, raw:files.readText(post.full_source), source:post.source, enabled:!!hexo.config.post_asset_folder }; },
    parse(raw) {
    ...
    async create(data) {
      ...
      const createdDate=data.date||new Date().toISOString();
      if(hexo.config.post_asset_folder&&!String(hexo.config.new_post_name||':title.md').endsWith('.md'))throw badRequest('post_asset_folder 开启时，new_post_name 需以 .md 结尾，以便 Hexo 识别文章','POST_NAME_PATTERN_INVALID');
      const filename = newPostName(hexo.config, slug, createdDate, data.frontMatter || {});
      ...
      const values = { title: data.title, date: createdDate, slug };
      let scaffold = null;
      ...
      if (hexo.config.post_asset_folder) files.mkdir(filePath.slice(0, -path.extname(filePath).length));
    ...
    async update(id, data) {
      ...
      const savedRaw = files.readText(filePath);
      const revision = contentRevision(savedRaw);
      const saveState = await refreshSavedContent(context);
      return { ...saveState, source: post.source, revision, raw:savedRaw };
    },
