...
function createPostService(context) {
  ...
  return {
    ...
    async create(data) {
      ...
      return { _id: created && created._id, published, path: path.relative(sourceDir, filePath), source: path.relative(hexo.base_dir, filePath), relative: relative.replace(/\\/g, '/'), revision: contentRevision(files.readBuffer(filePath)), workflowStatus: published ? 'published' : data.workflowStatus || 'draft' };
    },
