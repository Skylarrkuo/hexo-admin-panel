...
function createPostService(context) {
  ...
  return {
    list(query) {
      ...
      else if (query.status === 'draft') posts = posts.filter(post => post.published === false || post.isDraft);
    ...
    async create(data) {
      ...
      const content = buildPostSource({
        ...
        categories: data.categories || [], tags: data.tags || [], content: data.content || '', frontMatter: data.frontMatter || {}, published
      });
      ...
      return { _id: created && created._id, published, path: path.relative(sourceDir, filePath), source: path.relative(hexo.base_dir, filePath), relative: relative.replace(/\\/g, '/') };
    },
    ...
    async remove(id, revision) {
      ...
      const result = await context.services.trash.move('post', post.full_source, { title: post.title, source: post.source });
      if (context.services.scheduler) context.services.scheduler.completeForPost(id);
      return result;
    },
    ...
    async bulk(action, items) {
      ...
      for (const entry of prepared) {
        if (action === 'delete') results.push({ id: entry.item.id, result: await this.remove(entry.item.id, entry.item.revision) });
        else results.push({ id: entry.item.id, result: await this.publish(entry.item.id, action === 'publish', entry.item.revision) });
      }
    ...
    async publish(id, published, revision) {
      ...
      const next = value
        ? hexo.model('Post').find({}).toArray().find(item => path.resolve(item.full_source) === path.resolve(destination))
        : draftRecord(destination);
      if (value && context.services.scheduler) context.services.scheduler.completeForPost(id);
