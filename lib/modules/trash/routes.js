...
function trashRoutes(context) {
  ...
  return [
    { method: 'GET', path: '/trash', handler({ res, query }) { if(query.kind&&!['post','page','media'].includes(query.kind))throw badRequest('kind must be post, page or media','VALIDATION_ERROR');success(res,trash.list(query)); } },
