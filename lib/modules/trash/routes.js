...
function trashRoutes(context) {
  ...
  return [
    { method: 'GET', path: '/trash', handler({ res, query }) { if(query.kind&&!['post','media'].includes(query.kind))throw badRequest('kind must be post or media','VALIDATION_ERROR');success(res,trash.list(query)); } },
