...
function createTrashService(context) {
  ...
  function allowedOriginal(record) {
    ...
    if(record.kind==='post-asset')return isPathInside(context.paths.posts,record.originalPath)||isPathInside(context.paths.drafts,record.originalPath);
  ...
  return {
    async move(kind, sourcePath, metadata, options) {
      if (!['post', 'page', 'media', 'post-asset'].includes(kind)) throw badRequest('不支持的回收类型');
