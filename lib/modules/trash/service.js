...
function createTrashService(context) {
  ...
  function allowedOriginal(record) {
    const root = record.kind === 'post' ? context.paths.posts : record.kind === 'media' ? context.paths.images : null;
  ...
  return {
    async move(kind, sourcePath, metadata) {
      if (!['post', 'media'].includes(kind)) throw badRequest('不支持的回收类型');
