...
function writeCredentialState(statePath, state) {
  fs.mkdirSync(path.dirname(statePath), { recursive: true });
  writeTextFileAtomic(statePath, dumpYaml(state));
  try { fs.chmodSync(statePath, 0o600); } catch (_) {}
}

function scrubCredentialFile(configPath, document) {
  ...
  if (changed) writeTextFileAtomic(configPath, dumpYaml(document));
  return changed;
}
...
function loadAdminConfig(hexo) {
  ...
  if (usingDefaultAccount) {
    ...
    if (!(state && state.password_hash) && typeof config.password === 'string') {
      ...
      writeCredentialState(statePath, migrated);
      state = migrated;
  ...
  if (state && state.password_hash && fileDocument) {
    try {
      if (scrubCredentialFile(configPath, fileDocument)) {
