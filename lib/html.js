...
loadScriptFallback(['https://cdn.jsdelivr.net/npm/vue@3.5.13/dist/vue.global.prod.js','https://cdnjs.cloudflare.com/ajax/libs/vue/3.5.13/vue.global.prod.js','https://cdn.bootcdn.net/ajax/libs/vue/3.5.13/vue.global.prod.js']).then(function(){
  return loadScriptFallback(['https://cdn.jsdelivr.net/npm/marked@15.0.7/marked.min.js','https://cdnjs.cloudflare.com/ajax/libs/marked/15.0.7/marked.min.js','https://cdn.bootcdn.net/ajax/libs/marked/15.0.7/marked.min.js']);
}).then(function(){initApp();}).catch(function(){
  document.body.innerHTML='<div style="text-align:center;padding:60px;font-family:sans-serif"><h2 style="color:#ef4444">依赖加载失败</h2><p>无法加载 Vue.js 或 marked.js，请检查网络连接。</p><p style="color:#64748b;font-size:13px">已尝试: jsdelivr, cdnjs, bootcdn</p><button onclick="location.reload()" style="margin-top:16px;padding:8px 24px;cursor:pointer;border:1px solid #3b82f6;background:#3b82f6;color:#fff;border-radius:8px;font-size:14px">重试</button></div>';
});
function initApp(){
