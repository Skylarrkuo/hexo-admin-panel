...
const { createHistoryService } = require('./modules/history/service');
const { createCheckService } = require('./modules/checks/service');
const { createPublishingService } = require('./modules/publishing/service');
const { createRecoveryService } = require('./modules/history/recovery');
const { historyRoutes } = require('./modules/history/routes');
const { postAssetRoutes } = require('./modules/media/post-assets');
...
function createContext(hexo, config, runtime) {
  ...
  context.services.history = createHistoryService(context);
  context.repositories.files.observe(context.services.history);
  ...
  context.services.checks = createCheckService(context);
  context.services.publishing = createPublishingService(context);
  context.services.recovery = createRecoveryService(context);
  return context;
}
...
module.exports = function createApiHandler(hexo, config, runtime) {
  ...
  [
    ...
    scaffoldRoutes, pageRoutes, taxonomyRoutes, previewRoutes, systemRoutes, historyRoutes, postAssetRoutes
