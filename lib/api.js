...
module.exports = function createApiHandler(hexo, config) {
  ...
  addRoute('GET', '/config', (req, res) => {
    authMiddleware(req, res, () => {
      try {
        const configPath = hexo.config_path || path.join(hexo.base_dir, '_config.yml');
        const raw = fs.readFileSync(configPath, 'utf8');
        const parsed = yaml.load(raw);
        json(res, { success: true, data: { raw, parsed } });
      } catch (e) {
        json(res, { success: false, error: e.message }, 500);
      }
    }, config);
  ...
  addRoute('PUT', '/config', async (req, res) => {
    authMiddleware(req, res, async () => {
      try {
        const body = await parseJsonBody(req);
        if (!body.raw) return json(res, { success: false, error: 'No config content provided' }, 400);
