...
- **导航分组更清楚**：工作概览独立显示，其他入口按“内容管理、发布与维护、站点设置”分组；统一图标、间距和选中提示，矮屏下可独立滚动。
- **账户操作集中管理**：在侧栏底部的“账户与安全”中修改密码、退出登录或退出所有会话；语言、主题与查看站点收进紧凑工具栏。
- **随时修改登录密码**：输入当前密码、新密码与确认密码即可保存。新密码需为 12–256 个字符；保存后当前会话保持登录，其他会话需重新登录。
- **手机导航更方便**：底部保留概览、文章、发布和“全部导航”，其余功能在分组抽屉中打开；同步适配中英文、亮暗主题和键盘操作。

升级命令与步骤见[版本与发布](#版本与发布)，完整记录见 [CHANGELOG.md](./CHANGELOG.md)。

## 功能概览

### 内容创作

- 创建、编辑、发布、撤回和回收文章，支持全文检索与批量操作；标题下集中显示日期和字数，行内“更多”菜单提供发布、定时与删除等操作。
...
- 提供 Redefine 主题配置与 `source/_data/essays.yml` 随笔管理；单篇随笔在原卡片内编辑并实时预览，支持勾选批量编辑、粘贴拆分批量添加、统一时间和 Markdown 预览，每批最多 100 条，整批校验后一次保存。
- 站点配置、主题配置、文章、页面与随笔写入均带 revision，防止并发静默覆盖。

### 媒体资源

- 递归管理 `source/images` 及其子目录，以完整相对路径区分同名资源；卡片完整预览图片，分区展示文件信息、引用详情和操作按钮。
...
- 未初始化的账号首次登录后需设置正式密码；已有明文凭据自动迁移为 PBKDF2 哈希。日常可在“账户与安全 → 修改密码”中更改密码，并使其他会话失效。
...
- 普通退出撤销当前 token，侧栏“账户与安全 → 退出所有会话”持久化轮换签名密钥。退出失败时界面保留会话并提示重试，不把本地清除伪装成服务端撤销。
...
登录后台后，可随时从导航底部的「账户与安全 → 修改密码」输入当前密码、新密码和确认密码。新密码需为 12–256 个字符，且不能与当前密码相同。保存后当前会话保持登录，其他会话立即失效；新密码会持久保存，无需重启 Hexo。
...
在“文章管理”中搜索标题或正文，使用状态筛选定位内容。点击文章标题或“编辑”进入编辑器；点击“更多”可以发布、取消发布、设置或取消定时，以及删除文章。菜单可用 Esc 或点击外部关闭。勾选文章后，列表上方显示批量操作。

### 在原位置编辑随笔

在“随笔管理”中找到目标随笔并点击“编辑”，直接在该卡片内修改时间和内容、查看预览，然后点击“保存随笔”。需要一起修改多篇时，先勾选条目，再选择“批量编辑已选”；新内容可使用“写一篇随笔”或“批量添加”。
...
进入“恢复中心”，按文件名或备份类型筛选，点击对应行的“预览恢复差异”，在该备份下方比较当前文件与历史内容。确认后选择“确认恢复此文件”；恢复前会先备份当前文件。
...
当前版本为 `3.5.2`。项目遵循语义化版本规范，详细变更见 [CHANGELOG.md](./CHANGELOG.md)。
...
安装完成后重启 Hexo，再刷新 `/admin` 页面。3.5.2 不需要修改配置或迁移文章、随笔和媒体；保留现有 `.hexo-admin/` 目录即可继续使用登录状态、历史与备份。Node.js 版本要求仍为 20 或更高。
