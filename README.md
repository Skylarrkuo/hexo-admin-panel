...
- **随笔就地修改**：编辑器在当前随笔卡片内展开，边写边预览，保存后更新当前列表，不再跳到页首。
- **文章列表更易读**：增大标题与按钮字号，集中展示分类和标签；通过“编辑”和“更多”处理日常操作，手机端自动使用卡片布局。
- **媒体与恢复操作更舒展**：完整查看图片，展开引用详情；在对应备份下方核对恢复差异，长文件名和操作按钮有更充足的空间。
- **刷新登录更连贯**：先验证已有会话，再进入当前页面，避免登录窗口闪现。选择框、日期输入和菜单动效同步适配亮色、暗色与窄屏。
...
当前版本为 `3.5.1`。项目遵循语义化版本规范，详细变更见 [CHANGELOG.md](./CHANGELOG.md)。
...
安装完成后重启 Hexo，再刷新 `/admin` 页面。3.5.1 不需要修改配置或迁移文章、随笔和媒体；保留现有 `.hexo-admin/` 目录即可继续使用登录状态、历史与备份。Node.js 版本要求仍为 20 或更高。
